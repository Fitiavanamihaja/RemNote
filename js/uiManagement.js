...
if (!appData) {
  ...
  setTimeout(() => showToast("Erreur : Données de l'application non chargées", "error"), 0);
}
...
for (const [key, element] of Object.entries(elements)) {
  if (!element) {
    ...
    if (elements.toast) showToast(`Erreur : Élément ${key} non trouvé`, "error");
  }
}

// Color schemes for tags (persistent per tag)
const tagColors = {};
const colorSchemes = [
...
function getTagColor(tag) {
  if (!tagColors[tag]) {
    ...
    tagColors[tag] = isDark ? scheme.dark : scheme.light;
  }
...
function renderNotes() {
  try {
      ...
      sortedNotes.forEach((note) => {
        ...
        noteCard.innerHTML = `
          <div class="p-4">
            ...
            <p class="text-gray-700 dark:text-gray-100 mb-3 whitespace-normal break-words">${contentPreview.replace(/[#*_>-]/g, "")}</p>
            <div class="flex flex-wrap gap-1">
              ${note.tags.map((tag) => `<span class="${getTagColor(tag)} text-xs px-2 py-1 rounded">#${tag}</span>`).join("")}
            </div>
          </div>
        `;
    ...
    if (elements.toast) showToast(`Erreur lors du rendu des notes : ${error.message}`, "error");
  }
}
...
function renderTags() {
  try {
    ...
    if (elements.toast) showToast(`Erreur lors du rendu des tags : ${error.message}`, "error");
  }
}
...
window.openNoteInView = function (noteId) {
  try {
    ...
    if (!note) {
      ...
      if (elements.toast) showToast("Erreur : Note non trouvée", "error");
      return;
    }
    ...
    if (elements.viewTags) {
      elements.viewTags.innerHTML = note.tags
        .map((tag) => `<span class="${getTagColor(tag)} px-2 py-1 rounded text-xs">#${tag}</span>`)
        .join("");
    }
    ...
    if (elements.toast) showToast(`Erreur lors de l'ouverture de la note : ${error.message}`, "error");
  }
};
...
function openNoteInEditor(noteId) {
  try {
    ...
    if (elements.toast) showToast(`Erreur lors de l'ouverture de l'éditeur : ${error.message}`, "error");
  }
}
...
function createNewNote() {
  try {
    ...
    renderNotes();
    renderTags();
    if (elements.toast) showToast("Note créée avec succès", "success");
  } catch (error) {
    console.error("Error creating new note:", error);
    if (elements.toast) showToast(`Erreur lors de la création de la note : ${error.message}`, "error");
    renderNotes();
    renderTags();
  }
}
...
function saveCurrentNote() {
  try {
    ...
    if (noteId) {
      ...
      if (elements.toast) showToast("Note mise à jour avec succès", "success");
    } else {
    ...
    if (elements.toast) showToast(`Erreur lors de l'enregistrement de la note : ${error.message}`, "error");
  }
}
...
function goToNotesList() {
  ...
  if (appData) appData.setCurrentNoteId(null);
}
...
function addTagToEditor(tag) {
  ...
  tagElement.className = `tag-item ${getTagColor(tag)} px-2 py-1 rounded flex items-center gap-1`;
...
function updateMarkdownPreview() {
  try {
    ...
    if (elements.toast) showToast(`Erreur lors de la mise à jour de l'aperçu Markdown : ${error.message}`, "error");
  }
}
...
function openDeleteModal(noteId) {
  try {
    ...
    if (elements.toast) showToast(`Erreur lors de l'ouverture de la confirmation : ${error.message}`, "error");
  }
}
...
function closeDeleteModal() {
  try {
    ...
    if (elements.toast) showToast(`Erreur lors de la fermeture de la confirmation : ${error.message}`, "error");
  }
}
...
function deleteCurrentNote() {
  try {
    ...
    if (!noteId) {
      ...
      if (elements.toast) showToast("Erreur : Aucune note sélectionnée pour suppression", "error");
      closeDeleteModal();
      return;
    }
    const note = appData.notes.find((n) => n.id === noteId);
    ...
    renderNotes();
    renderTags();
    if (elements.toast) showToast("Note supprimée avec succès", "success");
    closeDeleteModal();
  } catch (error) {
    console.error("Error deleting note:", error);
    if (elements.toast) showToast(`Erreur lors de la suppression de la note : ${error.message}`, "error");
    closeDeleteModal();
  }
}
...
function showToast(message, type = "success") {
  try {
    ...
    if (type === "success") {
      ...
      elements.toast.className = "fixed bottom-4 right-4 bg-gray-800 dark:bg-gray-700 text-white px-4 py-3 rounded-lg shadow-lg";
    } else if (type === "error") {
      elements.toastIcon.className = "ri-error-warning-line";
      elements.toast.className = "fixed bottom-4 right-4 bg-red-500 text-white px-4 py-3 rounded-lg shadow-lg";
    } else if (type === "info") {
      elements.toastIcon.className = "ri-information-line";
      elements.toast.className = "fixed bottom-4 right-4 bg-blue-500 text-white px-4 py-3 rounded-lg shadow-lg";
    }
...
function formatText(type) {
  try {
    ...
    if (elements.toast) showToast(`Erreur lors du formatage du texte : ${error.message}`, "error");
  }
}
...
if (elements.editNoteBtn) {
  elements.editNoteBtn.addEventListener("click", function () {
    ...
    if (noteId) {
      ...
      if (elements.toast) showToast("Erreur : Aucune note sélectionnée pour modification", "error");
    }
  });
}
